...
const ms = require('ms'); // convert dates to ms for token expiry time
const validator = require('validator'); // library for checking email format
    ...
    try {
        ...
        if (existingUser) {
            if (existingUser.name === name) {
                throw new Error('There is already an account associated with that username.');
            }
            if (existingUser.email === email) {
                throw new Error('There is already an account associated with that email.');
            }
        }
        const saltRounds = 10;
